...
from PySide6.QtCore import QByteArray, QSettings, QTimer, Qt, QSize
...
from source.gui.app_icons import application_icon, toolbar_svg_icon
from source.gui.dialogs.about_dialog import AboutDialog
from source.gui.dialogs.filename_template_dialog import (
...
from source.gui.summary_tab_page import SummaryTabPage
from source.i18n.translator import Translator
from source.io.text_files import read_text_file_best_effort
...
class MainWindow(QMainWindow):
    ...
    _SETTING_LAST_PROJECT_DIR: str = "file/last_project_directory"
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._project_extract_dir: Path | None = None
    ...
    def _build_menu(self) -> None:
        ...
        open_project_action.triggered.connect(self._choose_open_project)
        file_menu.addAction(open_project_action)
        save_project_action = QAction("", self)
        save_project_action.triggered.connect(self._save_project)
        file_menu.addAction(save_project_action)
        import_action = QAction("", self)
        import_action.setShortcut(QKeySequence.Open)
        import_action.triggered.connect(self._choose_files)
        ...
        for lang in self._translator.ordered_ui_languages():
        ...
        self._action_import = import_action
        self._action_open_project = open_project_action
        self._action_save_project = save_project_action
    ...
    def _build_toolbar(self) -> None:
        """Add a top toolbar: project open/save, import, export; icon-only square buttons from ``resources/icons``."""
        tool_bar = QToolBar(self)
        tool_bar.setObjectName("mainToolBar")
        tool_bar.setMovable(False)
        tool_bar.setFloatable(False)
        _tb_icon: int = 30
        tool_bar.setIconSize(QSize(_tb_icon, _tb_icon))
        tool_bar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonIconOnly)
        for act, icon_stem in (
            (self._action_open_project, "Open"),
            (self._action_save_project, "Save"),
            (self._action_import, "Import"),
            (self._action_export, "Export"),
        ):
            ico = toolbar_svg_icon(icon_stem)
            if not ico.isNull():
                act.setIcon(ico)
    ...
    def _apply_window_title(self) -> None:
        """
        Set the main window title: base app name, with the current ``.mxap`` file name if any.
        """
        base = self._translator.tr_key("window.title")
        if self._project_path is not None:
            self.setWindowTitle(f"{base} — {self._project_path.name}")
        else:
            self.setWindowTitle(base)

    def _clear_project_extract_dir(self) -> None:
        """
        Remove the temp directory from the last ``load_mxap`` and clear the handle.
        """
        if self._project_extract_dir is not None and self._project_extract_dir.is_dir():
            try:
                shutil.rmtree(self._project_extract_dir, ignore_errors=True)
            except (OSError, TypeError) as exc:
                logger.warning("Could not remove project temp dir: %s", exc)
        self._project_extract_dir = None

    def _close_all_file_tabs(self) -> None:
        """
        Remove every per-file tab (``FileTabPage``) and resync the Summary tab.
        """
        for i in range(self._file_tabs.count() - 1, -1, -1):
            w = self._file_tabs.widget(i)
            if not isinstance(w, FileTabPage):
                continue
            s = w.session
    ...
    def _project_dialog_start_dir(self) -> str:
        """
        Default directory for open/save ``.mxap`` (last project dir, else last import dir, else empty).
        """
        store = QSettings()
        store.sync()
        raw = store.value(self._SETTING_LAST_PROJECT_DIR, "", type=str)
        if isinstance(raw, str) and raw.strip():
            try:
                d = Path(raw).expanduser().resolve(strict=False)
            except (OSError, ValueError) as exc:
                logger.debug("Invalid last project directory %r: %s", raw, exc)
            else:
                if d.is_dir():
                    return str(d)
        return self._file_dialog_start_directory()

    def _remember_project_dir(self, directory: Path) -> None:
        """
        Persist a folder for the next project file dialog.
        """
        if not isinstance(directory, Path):
            raise TypeError("directory must be pathlib.Path")
        try:
            ...
            logger.debug("Not persisting project directory: %s", exc)
            return
        if not resolved.is_dir():
            return
        store = QSettings()
        store.setValue(self._SETTING_LAST_PROJECT_DIR, str(resolved))
        store.sync()

    def _choose_open_project(self) -> None:
        """
        Open a ``.mxap`` archive: replace all file tabs, extract to a temp dir, restore optional UI language.
        """
        tr = self._translator.tr_key
        start = self._project_dialog_start_dir()
        path_str, _flt = QFileDialog.getOpenFileName(
            self,
            tr("menu.file.open_project"),
            start,
            tr("project.filter_mxap"),
        )
        if not path_str:
            return
        ...
        try:
            language, files = load_mxap(archive, new_root)
        except (MxapError, OSError) as exc:
            try:
                shutil.rmtree(new_root, ignore_errors=True)
            except OSError as rm:
                logger.debug("Cleanup after failed project open: %s", rm)
            logger.exception("Open project failed")
            QMessageBox.critical(self, tr("menu.file"), f"{tr('project.open_failed')}\n{exc!s}")
            return
        if not files:
            try:
                shutil.rmtree(new_root, ignore_errors=True)
            except OSError as rm:
                logger.debug("Cleanup: %s", rm)
            QMessageBox.information(
                self,
                tr("menu.file"),
                tr("project.open_no_files"),
            )
            return
        self._close_all_file_tabs()
        self._open_paths.clear()
        self._clear_project_extract_dir()
        self._project_extract_dir = new_root
        if language is not None:
            try:
                self._translator.set_language(language)
            except ValueError:
                logger.debug("Project language not available: %s", language)
        to_open = [f.path for f in files]
        self._open_resolved_paths(to_open)
        self._project_path = archive.resolve()
        self._apply_window_title()
        self._remember_project_dir(archive.parent)

    def _save_project(self) -> None:
        """
        Save all open ASAP files into a single ``.mxap`` (ZIP + metadata + sources). Current buffer text is used.
        """
        tr = self._translator.tr_key
        sessions = self._list_sessions_from_file_tabs()
        if not sessions:
            QMessageBox.information(
                self,
                tr("menu.file"),
                tr("project.save_empty"),
            )
            return
        entries: list[tuple[str, str, str]] = []
        for s in sessions:
            if not isinstance(s, OpenFileSession):
                continue
            entries.append((s.path.name, s.text, s.encoding))
        target = self._project_path
        if target is None or not target.is_file():
            f_mxap = tr("project.filter_mxap")
            out, _u = QFileDialog.getSaveFileName(
                self,
                tr("menu.file.save_project"),
                str(
                    Path(self._project_dialog_start_dir())
                    / "project.mxap"
                ),
                f_mxap,
                f_mxap,
            ...
            if not out:
                return
            target = Path(out)
            if target.suffix.lower() != ".mxap":
                target = target.with_suffix(".mxap")
        ...
        try:
            save_mxap(
                target,
                entries,
                self._translator.current_language,
            )
        except (OSError, MxapError) as exc:
            logger.exception("Save project failed")
            QMessageBox.critical(
                self,
                tr("menu.file"),
                f"{tr('project.save_failed')}\n{exc!s}",
            )
            return
        self._project_path = target.resolve()
        self._apply_window_title()
        self._remember_project_dir(self._project_path.parent)

    def _clear_project_path_for_loose_files(self) -> None:
        """
        Forget the on-disk project path when the user adds loose imports (file list is no longer a single .mxap).
        """
        if self._project_path is not None:
    ...
    def _refresh_menu_texts(self) -> None:
        ...
        t_open = tr("menu.file.open")
        t_open_p = tr("menu.file.open_project")
        t_save = tr("menu.file.save_project")
        t_ex = tr("menu.file.export")
        self._action_open_project.setText(t_open_p)
        self._action_open_project.setToolTip(t_open_p)
        self._action_save_project.setText(t_save)
        self._action_save_project.setToolTip(t_save)
        self._action_import.setText(t_open)
        self._action_import.setToolTip(t_open)
        self._action_export.setText(t_ex)
        self._action_export.setToolTip(t_ex)
    ...
    def _choose_files(self) -> None:
        ...
        self._clear_project_path_for_loose_files()
