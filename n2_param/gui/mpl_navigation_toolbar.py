...
from n2_param.gui.qt_figureoptions import figure_edit
...
class N2ParamNavigationToolbar2QT(NavigationToolbar2QT):
    """
    ...
    def edit_parameters(self) -> None:  # noqa: C901
        ...
        if not axes:
            QMessageBox.warning(
                ...
                if t is not None and hasattr(t, "tr_key")
                else N2ParamNavigationToolbar2QT._tr_errors_only_en("figure_options.error_title"),
                t.tr_key("figure_options.error_no_axes")
                if t is not None and hasattr(t, "tr_key")
                else N2ParamNavigationToolbar2QT._tr_errors_only_en("figure_options.error_no_axes"),
            )
            return
        if len(axes) == 1:
            ax, = axes
            ...
            item, ok = QInputDialog.getItem(
                ...
                if t is not None and hasattr(t, "tr_key")
                else N2ParamNavigationToolbar2QT._tr_errors_only_en("figure_options.customize"),
                t.tr_key("figure_options.select_axes")
                if t is not None and hasattr(t, "tr_key")
                else N2ParamNavigationToolbar2QT._tr_errors_only_en("figure_options.select_axes"),
                qtitles,
                0,
                False,
            )
